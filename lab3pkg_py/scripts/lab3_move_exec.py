...
class particleFilter:
    ...
    def __init__(self, dimension = 2, model = default_2D_Model(), numParticles = numberOfParticles, timeSpan = timsSpan, resamplingNoise = 0.01, positionStd = 5):
        ...
        for i in range(self.numParticles):
            x = np.random.uniform(self.curMin[0], self.curMax[0])
            y = np.random.uniform(self.curMin[1], self.curMax[1]) #self.curMin = [0, 0]
    ...
    def Measurement_Model(self, x, y):
        ...
        for particle in self.particles:
            error = 0
            for j in range(4):
                error += (self.model.readingSensor(x, y)[j] - self.model.readingMap(particle)[j])**2
            distance = (error**0.5)
        ...
        for i in range(len(self.weights)):
            self.weights[i] = self.weights[i] / sum_weights  
    ...
    def calcPosition(self):
        ...
        y = 0
        for i in range(len(self.particles)):
            x += self.weights[i] * self.particles[i][0]
            y += self.weights[i] * self.particles[i][1]
        #check boundary
        if x > self.curMax[0]:
            x = self.curMax[0]
        if x < self.curMin[0]:
